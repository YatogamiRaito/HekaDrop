...
fn lookup_tr(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "sender.text_summary" => "metin",
        "notify.text_sent_to" => "Metin gönderildi → {0}",
        "notify.text_empty" => "Panoda metin yok",
...
fn lookup_en(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "sender.text_summary" => "text",
        "notify.text_sent_to" => "Text sent → {0}",
        "notify.text_empty" => "Clipboard is empty",
        ...
        "webview.text.placeholder" => "Type or paste the text you want to send (Ctrl+V)",
        "webview.text.send" => "Send",
