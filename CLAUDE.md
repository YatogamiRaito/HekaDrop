...
- **missing_docs_in_private_items — hekadrop-net scope-limited** (PR `chore/lint-missing-docs-private-net`: PR #175 (`hekadrop-core`) pattern'inin küçük kardeşi. `crates/hekadrop-net/src/lib.rs`'e `clippy::missing_docs_in_private_items` mevcut `#![warn(...)]` listesine eklendi. **3 hit → 0 hit**: `discovery::parse` fn (mDNS `ResolvedService` → `DiscoveredDevice` parse), `mdns::MdnsHandle.daemon` field (`mdns-sd` daemon handle, Drop'ta unregister), `mdns::MdnsHandle.fullname` field (drop sırasında unregister parametresi). Workspace-wide skip — app/cli/proto hâlâ kapsam dışı. Doc-only, no behavior change.).
- **missing_docs_in_private_items — hekadrop-app scope-limited** (PR `chore/lint-missing-docs-private-app`: PR #175'in app crate kardeşi. `crates/hekadrop-app/src/{lib.rs,main.rs}` ayrı compilation unit olduğu için her ikisinin mevcut `#![warn(...)]` listesine `clippy::missing_docs_in_private_items` virgülle eklendi. 91 unique hit (87 source + 4 bench) → 0 hit. Etkilenen modüller: `paths` (3 path helper fn), `ui_adapter` (`PlatformAdapter`/`UiAdapter` struct + `new`), `i18n` (`Lang` enum + variants, `LANG` static, `detect`/`parse_lang`/`lookup_tr`/`lookup_en` fn), `state` (`STATE` static, `init`/`get` + 9 free-fn shim), `ui` (FileSummary/FolderSummary/AcceptResult struct field + variant, `format_payload_lines`, cfg-gated `prompt_accept_blocking`/`choose_files_blocking`/`choose_folder_blocking`/`choose_device_blocking`/`escape_applescript` her platform varyantına 1 satır), `main` (crate-level `//!`, RUNTIME static, WINDOW_HTML const, `async_main`/`cleanup_old_logs`/`run_app`/`handle_ipc`/`handle_settings_save` + Incoming struct fields, push_*_to_ui ailesi, `progress_signature`/`progress_label`/`push_progress_to_ui`, `initiate_send_flow`/`initiate_send_flow_with`/`initiate_text_send_flow`, `show_history`/`reveal_in_finder`/`open_downloads_folder`/`open_config_file`, `check_update_async`/`render_update_outcome`/`UpdateCheckOutcome` enum + variants + Available field'ları, `relative_time`/`human_size`/`toggle_login_item`). `benches/crypto.rs` `clippy::missing_docs_in_private_items` file-level allow ile pragmatik (criterion bench fn signature noise). Auto-fix sonrası `clippy::doc_markdown` 7 enforce hit (WebView/AppleScript/TTL backtick eksikliği) inline düzeltildi. Workspace-wide skip kalan: cli/proto. Doc-only, no behavior change. CI `-D warnings` ile fiili enforce.).
