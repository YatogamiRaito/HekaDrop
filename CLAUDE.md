...
- **missing_errors_doc + missing_panics_doc — hekadrop-net scope-limited** (PR `chore/lint-missing-docs-net`: PR #171 + #172'nin küçük kardeşi. `crates/hekadrop-net/src/lib.rs`'e `#![warn(clippy::missing_errors_doc, clippy::missing_panics_doc)]` source-level enable + crate-level scope yorumu. Toplam 2 hit (her ikisi `missing_errors_doc`; `missing_panics_doc` 0 hit): `discovery::scan` (mdns-sd `ServiceDaemon::new` / browse subscription hatası) + `mdns::advertise` (if_addrs / `ServiceInfo::new` / `ServiceDaemon::new` / register hatası). Doc-only, no behavior change. App/cli/proto hâlâ kapsam dışı. CI `-D warnings` ile fiili enforce.).
